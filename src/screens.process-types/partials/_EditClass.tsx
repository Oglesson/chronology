...
export const EditClass = () => {
	...
	const { editClassData, openEditClassModal, setOpenEditClassModal } =
	...
	useEffect(() => {
		if (openEditClassModal) {
			reset();
		}
	}, [openEditClassModal, reset]);
	...
	}, [responseData?.type, responseData, processResponse, setOpenEditClassModal]);

	return (
		<Modal isOpen={openEditClassModal}>
			<h2 className="typo-h3 mb-12">{t("editClass")}</h2>
			<fetcher.Form
				method="post"
				action=""
				onSubmit={(e) => {
			...
			>
				<input type="hidden" name="id" value={editClassData?.ID} />
				<input
					type="hidden"
					name={FORM_IDENTIFIERS.nameAttribute}
					value={FORM_IDENTIFIERS.updateProcessClass}
				/>
				<Input
					defaultValue={editClassData?.Description}
					error={errors.name}
					htmlFor={"name"}
					label={t("name")}
					name="name"
