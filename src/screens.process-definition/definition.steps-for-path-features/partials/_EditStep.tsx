...
export const EditStep = () => {
	...
	const [definitionStep, setDefinitionStep] =
		useState<ProcessDefinitionPathFeaturesStepData | null>(null);
	...
	useEffect(() => {
		if (
			...
			setDefinitionStep(
				definitionStepsForPathFeatures[
					Number(definitionStepActionIndex)
				]
			);
		}
	}, [definitionStepActionIndex, definitionStepsForPathFeatures]);
	...
	useEffect(() => {
		if (!definitionStep?.PathfeaturesStepsConditions?.length) {
			return;
		}
		...
		definitionStep.PathfeaturesStepsConditions.forEach(
			(condition) => {
				...
				<>
					...
					<form
						onSubmit={(e) => {
							...
							setDefinitionStepsForPathFeatures((previous) => {
								...
								const pathFeatureData: PathFeatureData =
									JSON.parse(pathFeature.toString());
					...
					>
						<div className="max-w-[23.5rem]">
							<AutoComplete
								key={`step-for-path-feature-${definitionStep._Feature?.Description}`}
								control={control}
								defaultValue={definitionStep._Feature}
