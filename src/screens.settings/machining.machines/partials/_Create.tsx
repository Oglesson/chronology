...
export const Create = () => {
	const { machineCodes } = useMachines();
	const machineFormSchema = useMachineFormSchema(machineCodes);
	...
	const onSubmit = (data: MachineFormData) => console.log(data);
	...
	useEffect(() => {
		window.requestAnimationFrame(() => {
			setCreateButton(
				<Button text={t("createAMachine")} onClick={setOpenModal} disabled={!canEdit} />
			);
		});
	...
	return (
		<>
			...
			<Modal isOpen={openModal}>
				<h2 className="typo-h3 mb-12">{t("createAMachine")}</h2>
				<fetcher.Form
					method="post"
					action=""
					onSubmit={(e) => {
				...
				>
					<input
						...
						value={FORM_IDENTIFIERS.createMachine}
					/>
					<Input
						error={errors.code}
						htmlFor={"code"}
					...
					<Select
						...
						label={t("department")}
						options={departmentOptionsWithNone}
					/>
